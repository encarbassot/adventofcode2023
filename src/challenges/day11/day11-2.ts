...
export default function (value:string,expansion = 1000000){

  const galaxys = parseInput(value,expansion)
...
export function parseInput(value:string,expansion=1){
  ...
  for(let i=0;i<grid.length;i++){
    ...
    const y = i + (overcomeGapsCol.length * (expansion-1))

    for(let j=0;j<grid[i].length;j++){

      const overcomeGapsRow = emptyColumns.filter(k=> k<j)
      const x = j + (overcomeGapsRow.length * (expansion-1))
